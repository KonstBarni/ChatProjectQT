    ...
    _activeUser(activeUser)
{
    ...
    //connect(*ui->banBut, &QAbstractButton::clicked, this, &MainWindow::on_banBut_clicked);
    fillMessagesForAll();
    fillPrivateMessages();
    _instancesCount++;
...
void MainWindow::fillMessagesForAll(QStringList messages)
{
    ...
    for(int i = 0; i < messages.size()-2; i+=3) {
        txt.append(line.arg(messages.at(i)).arg(_chat->getLoginById(messages[i+2].toStdString())).arg(messages.at(i+1)));
    }
...
void MainWindow::on_banBut_clicked_click_click()
{
...
void MainWindow::on_unBanBat_clicked()
{
    std::string q=ui->usersBox->currentText().toStdString();
    std::cout << "query:" << q << std::endl;
    _chat->unBanUser(q);
