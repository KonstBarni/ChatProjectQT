...
{
    QStringList result;
    QString query = "SELECT login, texting FROM message JOIN users ON message.id_sender = users.id "
                    "WHERE %1;";
...
void Chat::writeToOne(std::string text, std::shared_ptr<User> recipient)
{
    ...
    if (_database->hasConnection()) {
        std::string query = "SELECT id, banned FROM users WHERE login LIKE '" + getActiveUser()->getLogin() + "';";
        QVector<QString> ids = _database->queryResult(QString::fromStdString(query));
        std::string id_sender = ids.at(0).toStdString();

        std::string sender_banned = ids.at(1).toStdString(); //проверка на забанивание пользователя
        if(sender_banned=="T") return;

        query = "SELECT id, banned FROM users WHERE login LIKE '" + recipient->getLogin() + "';";
        ids = _database->queryResult(QString::fromStdString(query));
        std::string id_recipient = ids.at(0).toStdString();

        std::string recipient_banned = ids.at(1).toStdString(); //проверка на забанивание пользователя
        if(recipient_banned=="T") return;

        query = "INSERT INTO message (id_sender, id_receiver, texting) "
                "VALUES (" + id_sender + ", " + id_recipient + ", '" + text + "');";
        _database->executeQueryWithoutResult(QString::fromStdString(query));

        query = "SELECT id FROM message WHERE id_sender = " + id_sender + " AND id_receiver = " + id_recipient + " AND texting LIKE '" + text + "';";
        ids = _database->queryResult(QString::fromStdString(query));
...
void Chat::writeToAll(const std::string text)
{
    ...
    if (_database->hasConnection()) {
        ...
        //проверка на забанивание пользователя
        std::string sender_banned = ids.at(1).toStdString(); //проверка на забанивание пользователя
        if(sender_banned=="T") return;
